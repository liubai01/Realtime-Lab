...
void BaseImage::Upload(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
{
  ...
  device->GetCopyableFootprints(&mTextureDesc, 0, 1, 0, nullptr, nullptr, nullptr, &textureUploadBufferSize);
...
void BaseImage::AppendDescHeap(ID3D12Device* device, ID3D12DescriptorHeap* descHeap, int offset)
{
  ...
  device->CreateShaderResourceView(mTextureBuffer.Get(), &srvDesc, handle);
}
